...
def run_producer(server=None, port=None):
    ...
    for _, row in merged_df_till.iterrows(): # Management refers to soil management (tillage)
    ...
    for exp_no, meta in exp_no_to_meta.items():
        ...
        # - are not the specified crop, e.g. meta['Crop'] != 'WRa' skips all experiments that do not have WRa as crop
        # - have missing sowing or harvest dates
        # - are in the specified site name, e.g. meta['Name'] in ['ATB_Marquart', 'FI_Dahlhausen'] skips all experiments with these site names
        if (meta['Crop'] != 'WRa' or pd.isna(meta['Sowing'] or pd.isna(meta['Harvest'])) or meta['Name'] in
