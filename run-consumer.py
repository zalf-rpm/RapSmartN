...
def run_consumer(server=None, port=None):
    ...
    daily_filepath = f"{path_to_out_dir}/out.csv"
    with open(daily_filepath, "wt", newline="", encoding="utf-8") as daily_f:
        ...
        daily_writer.writerow([
            "Experiment", "Date", "DOY", "Year", "Yield", "AbBiom", "AbBiomNc", "SumNUp", "TempSum"
        ])
        ...
        while no_of_exps_to_receive != no_of_exps_received:
            try:
                ...
                for vals in results:
                    ...
                    row = [exp_no, vals["Date"], vals["DOY"], vals["Year"], vals["Yield"], vals["AbBiom"], vals["AbBiomNc"], vals["SumNUp"], vals["TempSum"]]
